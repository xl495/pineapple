...
function handleLogin() {
  formRef.value?.validate(async (errors) => {
    ...
    if (isRemember.value)
      local.set('loginEmail', { email, password })
    else local.remove('loginEmail')
...
function checkUserEmail() {
  const loginEmail = local.get('loginEmail')
  if (!loginEmail)
    return

  formValue.value = loginEmail
  ...
  <div>
    ...
    <n-form ref="formRef" :rules="rules" :model="formValue" :show-label="false" size="large">
      ...
      <n-space vertical :size="20">
        ...
        <n-flex>
          <n-text>{{ $t('login.noEmailText') }}</n-text>
